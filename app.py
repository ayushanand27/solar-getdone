...
with st.spinner("🌞 Loading Solar OS..."):
    ctx = setup_app()

    if "last_mqtt" not in st.session_state:
        st.session_state.last_mqtt = 0
    if time.time() - st.session_state.last_mqtt > 60:
        ...
        st.session_state.last_mqtt = time.time()
        st.session_state.last_mqtt_result = (success, msg)
    else:
        success, msg = st.session_state.get("last_mqtt_result", (False, "Cached"))
    ...
    history = st.session_state.radiation_history
    if not history or history[-1] != current_radiation:
        history.append(current_radiation)
    st.session_state.radiation_history = history[-2:]

    anomaly = detect_anomaly(st.session_state.radiation_history)
    if anomaly.get("anomaly"):
        st.warning(f"⚠️ **Anomaly Detected:** {anomaly['message']}")
...
""",
    ...
    st.divider()
    st.markdown(
        "Use the sidebar to navigate pages — **Shield**, **Energy**, **Forecast**, "
        "**Analytics**, and **Edge Node**."
    )
