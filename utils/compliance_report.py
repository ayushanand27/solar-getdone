...
ACTIONS = {
    ...
    "lightning_arrestor": "Inspect lightning arrestor grounding resistance (<10 Ohm required).",
...
def generate_compliance_pdf(farm_name, score, grade_label, statuses, report_date):
    ...
    title_style = ParagraphStyle(
        "Title", parent=styles["Heading1"], alignment=TA_CENTER, textColor=DARK, fontSize=22, fontName=PDF_FONT_BOLD
    )
    ...
    story = [
        pdf_paragraph("CEA 2026 Compliance Report", title_style),
        Spacer(1, 0.15 * inch),
        pdf_paragraph(f"Farm: <b>{farm_name}</b>", sub_style),
        pdf_paragraph(f"Date: {report_date}", sub_style),
        pdf_paragraph("Central Electricity Authority - Amendment 2026", sub_style),
        Spacer(1, 0.3 * inch),
        pdf_paragraph(f"<b>Compliance Score:</b> {score}/100 - {grade_label}", body),
        Spacer(1, 0.25 * inch),
    ]

    for category in CHECKLIST:
        story.append(pdf_paragraph(f"<b>{category['category']}</b>", body))
        story.append(Spacer(1, 0.1 * inch))
        rows = [[pdf_cell("Item"), pdf_cell("Status")]]
        for item in category["items"]:
            rows.append(
                [
                    pdf_cell(item["label"]),
                    pdf_cell(statuses.get(item["id"], item["default"])),
                ]
            )
    ...
    story.append(pdf_paragraph("<b>Action Items</b>", body))
    story.append(Spacer(1, 0.1 * inch))
    if alerts:
        for alert in alerts:
            story.append(
        ...
        story.append(pdf_paragraph("No open action items - fully compliant.", body))

    story.append(Spacer(1, 0.4 * inch))
    story.append(pdf_paragraph("<i>Generated by Solar OS - CEA 2026 Compliance Module</i>", sub_style))
